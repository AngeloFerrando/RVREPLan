...
- CONNECTOR: the connector used by RVReplan to connect with the real system (the connectors for the three case studies are available)
- DEJAVU: the path to the folder where Dejavu has been installed (where dejavu.jar is)
- PLANNER: the path to the planner

## Example of use for the three different case studies:
...
where DEJAVU and PLANNER need to be replaced with the actual paths.
...
where the N parameter denotes the number of failures to instroduce (5 is the maximum value in these specific examples).
